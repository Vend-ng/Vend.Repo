...
export const authChecker: AuthChecker<IContext> = async (
  ...
  const groupPermissions = await groups.reduce(
    async (perms: Promise<Permission[]>, g: Group): Promise<Permission[]> => [...(await perms), ...(await g.permissions)],
    Promise.resolve([])
  );
  const allPermissions = [...permissions, ...groupPermissions];
  const permMatchesRole = allPermissions.some((permission: Permission) =>
